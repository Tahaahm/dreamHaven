...
class UserBehaviorController extends Controller
{
    ...
    public function trackCompare(Request $request): JsonResponse
    {
     ...
     public function getPropertyPopularityBreakdown(string $propertyId): JsonResponse    {@@ -456,64 +399,44 @@ public function getPropertyPopularityBreakdown(string $propertyId): JsonResponse                return response()->json(['error' => 'Property not found'], 404);            }            $since30 = now()->subDays(30);            [$searchClicks, $searchImpressions, $compareCount, $velocity48h] =                collect([                    ['search_click'],                    ['search_impression'],                    ['compare'],                    ['view', 'search_click', 'favorite', 'compare'],                ])->map(                    fn($types) =>                    UserPropertyInteraction::where('property_id', $propertyId)                        ->whereIn('interaction_type', $types)                        ->where(                            'created_at',                            '>=',                            count($types) > 1 && in_array('view', $types)                                ? now()->subHours(48)                                : $since30                        )                        ->count()                )->all();            $clicks = UserPropertyInteraction::where('property_id', $propertyId)                ->where('interaction_type', 'search_click')                ->where('created_at', '>=', $since30)                ->get()                ->map(                    fn($row) => is_array($row->metadata)                        ? $row->metadata                        : json_decode($row->metadata, true)                );            $clickPositions = $clicks->pluck('result_position')->filter()->values();            $topQueries     = $clicks->pluck('query')->filter()->countBy()->sortDesc()->take(10);            $ctr            = $searchImpressions > 0                ? round(($searchClicks / $searchImpressions) * 100, 2)                : 0;            return response()->json([                'property_id' => $propertyId,                'period_days' => 30,@@ -529,7 +452,7 @@ public function getPropertyPopularityBreakdown(string $propertyId): JsonResponse                ],                'click_position_distribution' => $clickPositions->countBy()->sortKeys(),                'top_search_queries'          => $topQueries,                'status'                      => [                    'is_boosted' => (bool) $property->is_boosted,                    'verified'   => (bool) $property->verified,                    'created_at' => $property->created_at,
Collapse file‎app/Http/Controllers/PropertyController.php‎Copy file name to clipboardExpand all lines: app/Http/Controllers/PropertyController.php+69-57Lines changed: 69 additions & 57 deletionsOriginal file line numberOriginal file lineDiff line numberDiff line change@@ -1656,60 +1656,71 @@ public function getRecommended(Request $request)    {        try {            $limit = $request->get('limit', 20);            $user  = auth('sanctum')->user();            Log::info('🎯 RECOMMENDED: Request started', [                'endpoint'           => 'getRecommended',                'user_authenticated' => $user ? 'YES' : 'NO',                'user_id'            => $user?->id,                'limit'              => $limit,            ]);            // ── Cache TTL constants ───────────────────────────────────────────────            $authTtl  = 600;  // 10 min for authenticated users            $guestTtl = 600;  // 10 min for guests            if ($user) {                $cacheKey = "recommended_user_{$user->id}_{$limit}";                $properties = Cache::remember($cacheKey, $authTtl, function () use ($user, $limit) {                    $personalizedLimit = intval($limit * 0.7);                    try {                        $personalized = $this->interactionService->getPersonalizedRecommendations(                            $user->id,                            $personalizedLimit                        );                    } catch (\Exception $e) {                        Log::warning('Personalized recommendations failed, using fallback', [                            'error' => $e->getMessage(),                        ]);                        $personalized = collect();                    }                    $trendingLimit = $limit - $personalized->count();                    // ── Single query: trending fill ───────────────────────────────                    $trending = Property::query()                        ->where('is_active', true)                        ->where('published', true)                        ->whereNotIn('status', ['cancelled', 'pending', 'sold', 'rented'])                        ->whereNotIn('id', $personalized->pluck('id')->toArray())                        ->with('owner')   // ← eager-load here, not in a second query                        ->selectRaw('properties.*,                        (                            (CASE WHEN DATEDIFF(NOW(), created_at) <= 7 THEN 50 ELSE 0 END) +                            (views * 0.3) +                            (favorites_count * 2) +                            (CASE WHEN is_boosted = 1 THEN 30 ELSE 0 END) +                            (CASE WHEN verified  = 1 THEN 20 ELSE 0 END)                        ) as trending_score                    ')                        ->orderByDesc('trending_score')                        ->limit($trendingLimit)                        ->get();                    // ── Load owner for personalized (they didn't have it yet) ─────                    $personalized->load('owner');                    return $personalized->merge($trending)->values();                });                $personalized = collect(); // for log below — real counts are inside cache                $trending     = collect();            } else {                $cacheKey = "recommended_guest_{$limit}";                $properties = Cache::remember($cacheKey, $guestTtl, function () use ($limit) {                    return Property::query()                        ->where('is_active', true)                        ->where('published', true)@@ -1720,6 +1731,7 @@ public function getRecommended(Request $request)                                ->orWhere('views', '>', 50)                                ->orWhere('favorites_count', '>', 5);                        })                        ->with('owner')                        ->orderByDesc('is_boosted')                        ->orderByDesc('verified')                        ->orderByDesc('created_at')@@ -1730,38 +1742,38 @@ public function getRecommended(Request $request)            Log::info('✅ RECOMMENDED: Success', [                'properties_found' => $properties->count(),                'personalized'     => $user ? true : false,            ]);            // ── Impression tracking: fire-and-forget, don't block response ────────            if ($properties->isNotEmpty()) {                $userId = $user ? $user->id : 'guest_' . session()->getId();                dispatch(function () use ($userId, $properties) {                    $this->interactionService->trackImpressions(                        $userId,                        $properties,                        'recommended'                    );                })->afterResponse();            }            $transformedData = $properties->map(fn($p) => $this->transformPropertyData($p));            return ApiResponse::success(                'Recommended properties retrieved',                [                    'data'        => $transformedData,                    'total'       => $transformedData->count(),                    'personalized' => $user ? true : false,                    'algorithm'   => $user ? 'hybrid' : 'curated',                ],                200            );        } catch (\Exception $e) {            Log::error('❌ RECOMMENDED: Error', [                'message' => $e->getMessage(),                'file'    => $e->getFile(),                'line'    => $e->getLine(),            ]);            return ApiResponse::error('Failed to get recommended properties', $e->getMessage(), 500);        }
}