...
<head>
    ...
    <title>Contacts - Dream Mulk</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .contact-card { background: var(--bg-main); border: 1px solid var(--border-color); border-radius: 12px; padding: 24px; text-align: center; transition: all 0.3s; cursor: pointer; }
        .contact-card:hover { transform: translateY(-3px); box-shadow: 0 12px 40px var(--shadow); border-color: rgba(99,102,241,0.4); }
        .contact-avatar { width: 80px; height: 80px; border-radius: 50%; background: linear-gradient(135deg, #6366f1, #8b5cf6); display: flex; align-items: center; justify-content: center; color: white; font-weight: 700; font-size: 28px; margin: 0 auto 16px; }
        .contact-name { font-size: 18px; font-weight: 700; color: var(--text-primary); margin-bottom: 6px; }
        .contact-role { font-size: 13px; color: var(--text-muted); margin-bottom: 16px; }
        .contact-info { display: flex; flex-direction: column; gap: 8px; margin-bottom: 16px; }
        .contact-detail { font-size: 13px; color: var(--text-secondary); display: flex; align-items: center; gap: 8px; justify-content: center; }
        .contact-detail i { width: 16px; color: var(--text-muted); }
        .contact-actions { display: flex; gap: 8px; justify-content: center; }
        .btn-icon { width: 36px; height: 36px; border-radius: 8px; display: flex; align-items: center; justify-content: center; border: 1px solid var(--border-color); background: var(--bg-card); color: var(--text-secondary); cursor: pointer; transition: all 0.2s; }
...
<body>
    <div class="sidebar">
        ...
        <div class="nav-menu">
            ...
            <a href="{{ route('office.contacts') }}" class="nav-item active"><i class="fas fa-address-book"></i> Contacts</a>
    ...
    <div class="main-content">
        <div class="top-bar">
            ...
            <div class="top-actions">
                ...
                <button class="add-btn"><i class="fas fa-plus"></i> Add Contact</button>
            </div>
        </div>
            ...
            <div class="section">
                @if(isset($contacts) && $contacts->count() > 0)
                    <div class="contacts-grid">
                        ...
                        <div class="contact-card">
                            <div class="contact-avatar">{{ strtoupper(substr($contact->name, 0, 2)) }}</div>
                            <div class="contact-name">{{ $contact->name }}</div>
                            <div class="contact-role">{{ $contact->role ?? 'Client' }}</div>
                ...
                @else
                    <div class="empty">
                        ...
                        <p>Your contact directory will appear here</p>
                    </div>
                @endif
            </div>
        </div>
