...
@section('title', 'Command Center — DreamMulk')
...
@php
    ...
    // ── FIX: Build the greeting subtext in pure PHP, no inline Blade directives ──
    $riskAmount    = $pulse['revenue_at_risk'] ?? 0;
    $taskWord      = $totalTasks === 1 ? 'item needs' : 'items need';
    $greetingBody  = '';
    if ($totalTasks > 0) {
        $greetingBody = $totalTasks . ' ' . $taskWord . ' your decision';
        if ($riskAmount > 0) {
            $greetingBody .= ', and ' . number_format($riskAmount) . ' IQD of subscriptions expire within 7 days';
        }
        $greetingBody .= '.';
    } else {
        $greetingBody = 'Queue is clear — nothing is waiting on you right now.';
    }

    // ── Daily summary numbers for the hero pulse strip ──
    $heroPulse = [
        ['Coming back',    number_format($retention['repeat_visitors'] ?? 0), 'in 30 days',      'fa-rotate-right'],
        ['New this week',  number_format($pulse['listings_week'] ?? 0),       'listings',         'fa-house-circle-check'],
        ['Agent trust',    ($pulse['verify_rate'] ?? 0) . '%',               'verified agents',  'fa-shield-check'],
        ['Avg. asking',    '$' . number_format($pulse['avg_price'] ?? 0),    'per listing',      'fa-tag'],
        ['Online now',     number_format($visitors['live'] ?? 0),            'visitors',         'fa-circle-dot'],
        ['Boosted',        number_format($pulse['boosted'] ?? 0),            'listings',         'fa-rocket'],
    ];
@endphp
...
<style>
    ...
    .dm-hero {
        background:
            radial-gradient(1200px 380px at 12% -35%, rgba(75,86,178,.6), transparent 58%),
            radial-gradient(900px 500px at 85% 120%, rgba(139,92,246,.25), transparent 55%),
            linear-gradient(140deg, #11142b 0%, #1e2247 50%, #2a347f 100%);
    ...
    .card-ttl { font-weight: 800; color: var(--ink); font-size: 14.5px; letter-spacing: -.015em; }
    ...
    .kpi-spark { height: 56px; overflow: hidden; }

    .chip { display: inline-flex; align-items: center; gap: 3px; font-size: 10.5px; font-weight: 800; padding: 3px 8px; border-radius: 999px; }
    ...
    .task-row:hover { background: rgba(255,255,255,.08); }

    /* ─── Queue items ─────────────────────────────────────────────── */
    .queue-item { display: flex; gap: 13px; padding: 13px 16px; border-bottom: 1px solid #f4f5f9; }
    .queue-thumb { width: 64px; height: 58px; border-radius: 12px; object-fit: cover; background: #eef0f7; flex-shrink: 0; }

    /* ─── Radar / leaderboard rows ────────────────────────────────── */
    .radar-item, .lb-row {
        display: flex; align-items: center; gap: 11px;
        padding: 11px 16px; border-bottom: 1px solid #f4f5f9;
        text-decoration: none; transition: background .12s;
    }
    .lb-row { border-bottom: 0; border-radius: 12px; padding: 9px 11px; }
    .lb-row:hover { background: #f5f6fb; }
    ...
    .st-critical { background: #f97316; }
    .st-soon     { background: var(--amber); }
    .st-ok       { background: #22c55e; }
    .tx-expired  { color: #b91c1c; background: #fef2f2; }
    .tx-critical { color: #c2410c; background: #fff7ed; }
    .tx-soon     { color: #a16207; background: #fefce8; }
    .tx-ok       { color: #15803d; background: #f0fdf4; }

    .rank { width: 20px; text-align: center; font-weight: 900; font-size: 12px; color: #cbd5e1; }
    ...
    .seg button { border: 0; background: transparent; font-size: 11px; font-weight: 800; color: #64748b; padding: 6px 11px; border-radius: 8px; cursor: pointer; transition: .15s; min-height: 32px; }
    .seg button.on { background: #fff; color: var(--brand); box-shadow: 0 1px 3px rgba(15,23,42,.12); }

    .tabs { display: flex; gap: 14px; overflow-x: auto; }
    .tabs::-webkit-scrollbar { display: none; }
    .tabs button { border: 0; background: transparent; font-size: 12px; font-weight: 800; color: var(--ink-3); padding: 6px 0; cursor: pointer; border-bottom: 2px solid transparent; white-space: nowrap; }
    .tabs button.on { color: var(--brand); border-color: var(--brand); }

    /* ─── Buttons ─────────────────────────────────────────────────── */
    .ghost-btn {
        display: inline-flex; align-items: center; gap: 6px;
        font-size: 11.5px; font-weight: 800; padding: 8px 13px;
        border-radius: 11px; border: 1px solid var(--border); color: var(--ink-2);
        background: #fff; transition: .15s; min-height: 38px; text-decoration: none; cursor: pointer;
    }
    .ghost-btn:hover { border-color: var(--brand); color: var(--brand); background: #f5f6fd; }

    .solid-btn {
        display: inline-flex; align-items: center; gap: 7px;
        font-size: 12.5px; font-weight: 800; padding: 11px 18px;
        border-radius: 12px; color: #fff; min-height: 44px;
        background: linear-gradient(135deg, var(--brand), var(--brand-lt));
        box-shadow: 0 10px 22px -12px rgba(48,59,151,.9); transition: .16s;
        text-decoration: none; border: none; cursor: pointer;
    }
    .solid-btn:hover { background: linear-gradient(135deg, #232d8a, #3b47a5); transform: translateY(-1px); }
    .solid-btn:active { transform: scale(.97); }

    /* ─── Retention gauge ─────────────────────────────────────────── */
    .ret-tile { border: 1px solid #eceef6; border-radius: 16px; padding: 15px; background: #fff; }
    .ret-tile b { display: block; font-size: 25px; font-weight: 900; color: var(--ink); line-height: 1; margin-bottom: 5px; }
    ...
    .cohort th { font-size: 9.5px; font-weight: 800; text-transform: uppercase; letter-spacing: .1em; color: var(--ink-3); padding-bottom: 3px; }
    ...
    .cohort td.head { text-align: left; background: transparent; font-weight: 800; color: var(--ink); white-space: nowrap; padding-left: 0; }
    ...
    .hours span.peak { background: var(--brand); }

    /* ─── Revenue split bar ───────────────────────────────────────── */
    .rev-bar { height: 10px; border-radius: 999px; overflow: hidden; display: flex; background: #f1f2f7; }

    /* ─── Jump chips (mobile) ─────────────────────────────────────── */
    .jump { display: flex; gap: 8px; overflow-x: auto; padding: 2px 0 4px; scrollbar-width: none; }
    .jump::-webkit-scrollbar { display: none; }
    .jump a { flex: 0 0 auto; font-size: 11.5px; font-weight: 800; padding: 8px 14px; border-radius: 999px; background: #fff; border: 1px solid var(--border); color: var(--ink-2); text-decoration: none; }
    ...
    .pulse-stat:hover { background: rgba(255,255,255,.11); }

    /* ─── Hero greeting text with live dot ───────────────────────── */
    .live-dot {
        display: inline-block; width: 8px; height: 8px; border-radius: 50%;
        background: #22c55e; margin-right: 5px; position: relative; top: -1px;
    ...
    /* ─── Empty states ────────────────────────────────────────────── */
    .empty { padding: 30px 18px; text-align: center; color: var(--ink-3); font-size: 12.5px; font-weight: 600; }
    .empty i { display: block; font-size: 24px; margin-bottom: 9px; color: #dbe0ee; }

    /* ─── Scrollbar (custom) ──────────────────────────────────────── */
    .custom-scrollbar::-webkit-scrollbar { width: 5px; }
    .custom-scrollbar::-webkit-scrollbar-track { background: transparent; }
    .custom-scrollbar::-webkit-scrollbar-thumb { background: #dde0ea; border-radius: 99px; }
...
@section('content')
<div class="max-w-[1680px] mx-auto px-1">

{{-- ══════════════════════════════════════════════════════════════════
     1 · HERO — fixed greeting bug, enriched with 6-stat pulse strip
════════════════════════════════════════════════════════════════════ --}}
<div class="dm-hero dm-hero-grain mb-5 md:mb-7">
    <div class="relative z-10 p-5 md:p-8 lg:p-10">
        <div class="flex flex-col xl:flex-row xl:items-end justify-between gap-7">

            {{-- Left: greeting + actions --}}
            <div class="min-w-0">
                <p class="eyebrow text-white/40 mb-2">
                    {{ now()->format('l, d F Y') }} &nbsp;·&nbsp; Erbil, Kurdistan
                    @if(($visitors['live'] ?? 0) > 0)
                        &nbsp;·&nbsp; <span class="live-dot"></span>{{ number_format($visitors['live']) }} online now
                    @endif
                </p>

                <h1 class="text-[28px] md:text-[40px] font-black tracking-tight leading-none mb-3 text-white">
                    {{ $greeting }}, {{ $firstName }}
                </h1>

                {{-- ✅ FIX: pure PHP variable — no inline @if inside strings --}}
                <p class="text-white/65 text-[13px] md:text-[15px] font-medium max-w-2xl leading-relaxed">
                    @if($totalTasks > 0)
                        <span class="text-white font-black">{{ $totalTasks }}</span>
                        {{ $totalTasks === 1 ? 'item needs' : 'items need' }} your decision
                        @if($riskAmount > 0)
                            , and <span class="text-amber-300 font-black num">{{ number_format($riskAmount) }} IQD</span>
                            of subscriptions expire within 7 days
                        @endif.
                    @else
                        Queue is clear &mdash; nothing is waiting on you right now.
                    @endif
                </p>

                {{-- Quick-action bar --}}
                <div class="flex flex-wrap items-center gap-2.5 mt-6">
                    <a href="{{ $link('admin.properties.create') }}" class="solid-btn">
                        <i class="fas fa-plus"></i> Add property
                    </a>
                    <a href="{{ $link('admin.agents.create') }}" class="ghost-btn !bg-white/10 !border-white/15 !text-white">
                        <i class="fas fa-user-tie"></i> Add agent
                    </a>
                    <a href="{{ $link('admin.offices.create') }}" class="ghost-btn !bg-white/10 !border-white/15 !text-white hidden sm:inline-flex">
                        <i class="fas fa-building"></i> Add office
                    </a>
                    <a href="{{ $link('admin.projects.create') }}" class="ghost-btn !bg-white/10 !border-white/15 !text-white hidden md:inline-flex">
                        <i class="fas fa-city"></i> Add project
                    </a>
                    <a href="{{ url('admin/notifications/broadcast') }}" class="ghost-btn !bg-white/10 !border-white/15 !text-white hidden lg:inline-flex">
                        <i class="fas fa-bullhorn"></i> Broadcast
                    </a>
                    <a href="{{ route('admin.dashboard') }}?refresh=1"
                       class="ghost-btn !bg-transparent !border-white/15 !text-white/50"
                       title="Recalculate cached figures">
                        <i class="fas fa-rotate"></i>
                    </a>
                </div>

                {{-- Pending badges strip --}}
                @if($totalTasks > 0)
                <div class="flex flex-wrap gap-2 mt-4">
                    @if($pendingApprovals['properties'] > 0)
                        <a href="{{ $link('admin.properties.index', ['status' => 'pending']) }}"
                           class="inline-flex items-center gap-1.5 text-[10.5px] font-black px-3 py-1.5 rounded-full bg-red-500/20 border border-red-400/30 text-red-300 hover:bg-red-500/30 transition">
                            <i class="fas fa-house-circle-exclamation"></i> {{ $pendingApprovals['properties'] }} properties
                        </a>
                    @endif
                    @if($pendingApprovals['agents'] > 0)
                        <a href="{{ $link('admin.agents.index', ['status' => 'pending']) }}"
                           class="inline-flex items-center gap-1.5 text-[10.5px] font-black px-3 py-1.5 rounded-full bg-amber-500/20 border border-amber-400/30 text-amber-300 hover:bg-amber-500/30 transition">
                            <i class="fas fa-user-tie"></i> {{ $pendingApprovals['agents'] }} agents
                        </a>
                    @endif
                    @if($pendingApprovals['offices'] > 0)
                        <a href="{{ $link('admin.offices.index', ['status' => 'pending']) }}"
                           class="inline-flex items-center gap-1.5 text-[10.5px] font-black px-3 py-1.5 rounded-full bg-purple-500/20 border border-purple-400/30 text-purple-300 hover:bg-purple-500/30 transition">
                            <i class="fas fa-building"></i> {{ $pendingApprovals['offices'] }} offices
                        </a>
                    @endif
                    @if($pendingApprovals['banners'] > 0)
                        <a href="{{ $link('admin.banners.index', ['status' => 'pending']) }}"
                           class="inline-flex items-center gap-1.5 text-[10.5px] font-black px-3 py-1.5 rounded-full bg-blue-500/20 border border-blue-400/30 text-blue-300 hover:bg-blue-500/30 transition">
                            <i class="fas fa-rectangle-ad"></i> {{ $pendingApprovals['banners'] }} banners
                        </a>
                    @endif
                    @if($pendingApprovals['reports'] > 0)
                        <a href="{{ $link('admin.reports.index') }}"
                           class="inline-flex items-center gap-1.5 text-[10.5px] font-black px-3 py-1.5 rounded-full bg-rose-500/20 border border-rose-400/30 text-rose-300 hover:bg-rose-500/30 transition">
                            <i class="fas fa-flag"></i> {{ $pendingApprovals['reports'] }} reports
                        </a>
                    @endif
                </div>
                @endif
            ...
            {{-- Right: 6-stat pulse strip --}}
            <div class="grid grid-cols-3 gap-2.5 lg:grid-cols-3 xl:grid-cols-2 xl:gap-3 shrink-0 xl:min-w-[280px]">
                @foreach($heroPulse as [$label, $value, $sub, $icon])
                <div class="pulse-stat">
                    <div class="flex items-center justify-between mb-2">
                        <p class="eyebrow text-white/35">{{ $label }}</p>
                        <i class="fas {{ $icon }} text-white/25 text-[10px]"></i>
                    </div>
                    <p class="text-[22px] md:text-[26px] font-black num leading-none text-white">{{ $value }}</p>
                    <p class="text-[9.5px] text-white/40 font-bold mt-1.5 uppercase tracking-wide">{{ $sub }}</p>
                </div>
                @endforeach
            </div>
...
{{-- Mobile jump chips --}}
<div class="jump lg:hidden mb-5">
    <a href="#sec-kpi">Numbers</a>
...
{{-- ══════════════════════════════════════════════════════════════════
     2 · KPI ROW — 4 cards with sparklines + deltas
════════════════════════════════════════════════════════════════════ --}}
@php
    $kpis = [
        [
            ...
            'foot'  => $stats['active_subscriptions'] . ' active plans',
        ...
        [
            ...
            'foot'  => $stats['properties_for_sale'] . ' sale · ' . $stats['properties_for_rent'] . ' rent',
        ...
        [
            ...
            'color' => '#f59e0b',
            'delta' => null,
            'foot'  => $stats['total_agents'] . ' agents · ' . $stats['total_offices'] . ' offices',
            'href'  => $link('admin.agents.index'),
        ],
    ];
@endphp

<div id="sec-kpi" class="grid grid-cols-2 xl:grid-cols-4 gap-3 md:gap-5 mb-5 md:mb-7 scroll-mt-24">
    @foreach($kpis as $k)
    <a href="{{ $k['href'] }}" class="kpi-card card p-4 md:p-6 block relative overflow-hidden">
        <div class="flex items-start justify-between mb-4">
            <div class="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl grid place-items-center text-sm"
        ...
        <h3 class="text-[24px] md:text-[34px] font-black text-slate-900 num leading-none mb-1">{{ $k['value'] }}</h3>
        <p class="text-[10px] md:text-[11px] font-bold text-slate-400 uppercase tracking-wide">{{ $k['unit'] }}</p>

        <div class="mt-4 pt-3 border-t border-slate-100">
            <p class="text-[10.5px] md:text-[11.5px] text-slate-500 font-semibold leading-snug">{{ $k['foot'] }}</p>
        </div>

        <div class="kpi-spark absolute left-0 right-0 bottom-0 opacity-60 pointer-events-none" id="spark-{{ $k['key'] }}"></div>
    </a>
    @endforeach
</div>

{{-- ══════════════════════════════════════════════════════════════════
     3 · ACTION CENTER + GROWTH CHART
════════════════════════════════════════════════════════════════════ --}}
@php
    $tasks = [
        ['Properties',   $pendingApprovals['properties'],   'fa-house-circle-exclamation', $link('admin.properties.index', ['status' => 'pending']), '#ef4444'],
        ['Agents',       $pendingApprovals['agents'],       'fa-user-tie',                 $link('admin.agents.index',     ['status' => 'pending']), '#f59e0b'],
        ['Offices',      $pendingApprovals['offices'],      'fa-building',                 $link('admin.offices.index',    ['status' => 'pending']), '#8b5cf6'],
        ['Banners',      $pendingApprovals['banners'],      'fa-rectangle-ad',             $link('admin.banners.index',    ['status' => 'pending']), '#3b82f6'],
        ['Providers',    $pendingApprovals['providers'],    'fa-tools',                    $link('admin.service-providers.index'),                  '#06b6d4'],
        ['Appointments', $pendingApprovals['appointments'], 'fa-calendar-check',           $link('admin.appointments.index', ['status' => 'pending']), '#10b981'],
        ['Reports',      $pendingApprovals['reports'],      'fa-flag',                     $link('admin.reports.index'),                            '#ef4444'],
    ];
@endphp

<div id="sec-tasks" class="grid grid-cols-1 xl:grid-cols-3 gap-4 md:gap-6 mb-5 md:mb-7 scroll-mt-24">

    {{-- Action center --}}
    <div class="rounded-[20px] p-5 md:p-6 text-white relative overflow-hidden xl:order-2"
         style="background: linear-gradient(165deg, #1a1d2e 0%, #12141f 100%);">
        <div class="absolute -top-20 -right-16 w-56 h-56 rounded-full blur-3xl opacity-40" style="background:#303b97"></div>
        <div class="absolute bottom-0 left-0 w-40 h-40 rounded-full blur-3xl opacity-20" style="background:#8b5cf6"></div>
        ...
        <div class="relative z-10">
            <div class="flex items-center justify-between mb-1">
                ...
                @else
                <i class="fas fa-check-circle text-emerald-400 text-[13px]"></i>
                @endif
            </div>
            <h3 class="text-xl md:text-2xl font-black mb-1 text-white">Action center</h3>
            <p class="text-[11px] text-white/35 font-semibold mb-5">
                {{ $totalTasks > 0 ? $totalTasks . ' items need a decision' : 'All clear — nothing pending' }}
            </p>

            <div class="space-y-0.5 -mx-2">
                @foreach($tasks as [$label, $count, $icon, $url, $color])
                <a href="{{ $url }}" class="task-row">
                    <span class="w-8 h-8 rounded-xl grid place-items-center text-[11px] shrink-0"
                          style="background:{{ $count > 0 ? $color . '30' : 'rgba(255,255,255,.05)' }};color:{{ $count > 0 ? $color : 'rgba(255,255,255,.25)' }}">
                        <i class="fas {{ $icon }}"></i>
                    </span>
                    <span class="flex-1 text-[13px] font-bold {{ $count > 0 ? 'text-white/85' : 'text-white/30' }}">{{ $label }}</span>
                    <span class="num font-black text-[15px] {{ $count > 0 ? 'text-white' : 'text-white/20' }}">{{ $count }}</span>
                    @if($count > 0)
            ...
            @if($totalTasks === 0)
            <div class="mt-5 rounded-2xl bg-emerald-500/10 border border-emerald-400/20 px-4 py-3">
                <p class="text-[12.5px] font-bold text-emerald-300">
                    <i class="fas fa-check-circle mr-1.5"></i> Everything has been reviewed.
                </p>
            </div>
            @endif
            ...
            @if($riskAmount > 0)
            <div class="mt-4 rounded-2xl bg-amber-500/10 border border-amber-400/20 px-4 py-3">
                <p class="text-[11px] font-black text-amber-300 mb-0.5">
                    <i class="fas fa-triangle-exclamation mr-1.5"></i> Revenue at risk
                </p>
                <p class="text-[13px] font-black text-white num">{{ number_format($riskAmount) }} <span class="text-white/50 font-bold text-[11px]">IQD</span></p>
                <p class="text-[10px] text-white/35 font-semibold mt-0.5">Subscriptions expiring within 7 days</p>
            </div>
            @endif
        </div>
    </div>
...
{{-- ══════════════════════════════════════════════════════════════════
     4 · VISITORS
════════════════════════════════════════════════════════════════════ --}}
<div id="sec-visitors" class="card mb-5 md:mb-7 overflow-hidden scroll-mt-24">
    <div class="card-hd">
        <div><p class="eyebrow mb-1">Traffic</p><h3 class="card-ttl">Visitors & guests</h3></div>
        @if(($visitors['available'] ?? false) && ($visitors['live'] ?? 0) > 0)
            <span class="inline-flex items-center gap-1.5 text-[10.5px] font-black px-2.5 py-1 rounded-lg bg-emerald-50 text-emerald-700">
                <span class="live-dot" style="width:6px;height:6px;margin-right:2px"></span>
    ...
    <div class="p-4 md:p-6">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            <div>
                <div class="flex items-end gap-3 mb-2">
                    <p class="text-[44px] md:text-[56px] font-black text-slate-900 num leading-none">{{ number_format($visitors['today']) }}</p>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div class="rounded-2xl p-4 text-white" style="background:linear-gradient(140deg,#303b97,#4b56b2)">
                        ...
                        <p class="text-[10px] font-bold text-white/50 mt-1">{{ $visitors['return_rate'] }}% of today</p>
                    </div>
                    <div class="rounded-2xl p-4 border border-slate-200">
                        ...
                        <p class="text-[10px] font-bold text-slate-400 mt-1">brand new today</p>
                    </div>
                </div>
            ...
            <div class="lg:col-span-2">
                <div class="flex items-center justify-between mb-2 flex-wrap gap-2">
                    <h4 class="text-[13px] font-extrabold text-slate-900">Returning vs first-time (14 days)</h4>
        ...
        <div class="pt-5 border-t border-slate-100">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
                @php
                    $loyalty = [
                        ['Came back',         number_format($visitors['repeat_30']),      'visited 2+ days',    'fa-rotate-right'],
                        ['Regulars',          number_format($visitors['loyal_30']),       'visited 4+ days',    'fa-heart'],
                        ['Guest returners',   number_format($visitors['guest_repeat_30']),'never signed in',    'fa-user-secret'],
                        ['Avg. visits each',  $visitors['avg_days'],                     'days per person',    'fa-calendar-check'],
                        ['Converted',         $visitors['converted_30'] . '%',           'made an account',    'fa-user-plus'],
        ...
        {{-- Signed-in vs guests + app vs web --}}
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-5 pt-5 border-t border-slate-100">
            <div>
            ...
            <div>
                <h4 class="text-[13px] font-extrabold text-slate-900 mb-2">App vs web today</h4>
                @php $totalDevice = max(($visitors['app'] + $visitors['web']), 1); $appShare = (int) round(($visitors['app'] / $totalDevice) * 100); @endphp
                <div class="flex h-3 rounded-full overflow-hidden bg-slate-100 mb-2.5">
                    <div style="width:{{ $appShare }}%;background:#8b5cf6"></div>
                    <div style="width:{{ 100 - $appShare }}%;background:#e2e6f3"></div>
                </div>
                <div class="flex items-center gap-4 text-[11px] font-bold text-slate-500">
                    <span class="flex items-center gap-1.5"><i class="fas fa-mobile-screen" style="color:#8b5cf6"></i> {{ number_format($visitors['app']) }} app</span>
                    <span class="flex items-center gap-1.5"><i class="fas fa-globe text-slate-400"></i> {{ number_format($visitors['web']) }} web</span>
                </div>
            </div>
        </div>
        ...
        <div class="mt-5 pt-5 border-t border-slate-100">
            ...
            <div class="space-y-2">
                ...
                <div class="flex items-center gap-3">
                    <span class="text-[11.5px] font-bold text-slate-600 truncate w-36 md:w-72">/{{ ltrim($page['path'], '/') }}</span>
...
{{-- ══════════════════════════════════════════════════════════════════
     5 · RETENTION
════════════════════════════════════════════════════════════════════ --}}
<div id="sec-retention" class="card mb-5 md:mb-7 overflow-hidden scroll-mt-24">
    <div class="card-hd">
        <div><p class="eyebrow mb-1">Loyalty</p><h3 class="card-ttl">Who comes back</h3></div>
    ...
    @if(!($retention['available'] ?? false))
        <div class="empty"><i class="fas fa-chart-simple"></i>Nothing to measure yet. Switch on visitor tracking.</div>
    @else
    ...
    <div class="px-4 md:px-6 py-2.5 bg-amber-50 border-b border-amber-100 flex items-center gap-2.5">
        ...
        <p class="text-[11.5px] font-bold text-amber-800">Signed-in people only. Install the visits migration to include guests.</p>
    </div>
    @endif

    <div class="p-4 md:p-6">
        @php $g = $retention['groups']['everyone']; $circ = 2 * M_PI * 54; $dash = round((max(0,min(100,$g['rate']))/100)*$circ,2); @endphp
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 grid grid-cols-2 md:grid-cols-3 gap-3">
                ...
                @endphp
                @foreach($retTiles as [$id, $value, $label, $sub])
                <div class="ret-tile">
...
<div class="grid grid-cols-1 xl:grid-cols-3 gap-4 md:gap-6 mb-5 md:mb-7">
    <div class="xl:col-span-2 card overflow-hidden">
        <div class="card-hd">
            <div><p class="eyebrow mb-1">Cohorts</p><h3 class="card-ttl">Do new signups stay?</h3></div>
            <span class="text-[10.5px] font-bold text-slate-400">% still active each week</span>
        </div>
        @if(count($cohorts))
        <div class="p-4 overflow-x-auto custom-scrollbar">
            <table class="cohort">
                <thead>
                    <tr>
                        <th class="text-left">Signed up</th><th>Users</th>
                        @for($w = 0; $w <= 5; $w++)<th>Wk {{ $w }}</th>@endfor
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        @for($w = 0; $w <= 5; $w++)
                            ...
                            @if($val === null)<td class="void"></td>
                            @else<td class="num" style="background:rgba(48,59,151,{{ max(0.06,min(1,$val/100)) }});color:{{ $val > 45 ? '#fff' : '#334155' }}">{{ $val }}%</td>
                            @endif
                        @endfor
                    </tr>
                    @endforeach
            ...
            <p class="text-[11px] text-slate-400 font-semibold mt-3">Each row shows: of everyone who signed up that week, the share still active weeks later.</p>
        </div>
        @else
    ...
    <div class="card overflow-hidden">
        <div class="card-hd">
            <div><p class="eyebrow mb-1">Win back</p><h3 class="card-ttl">Gone quiet</h3></div>
        ...
        <div class="p-2 max-h-[330px] overflow-y-auto custom-scrollbar">
            ...
            <a href="{{ $link('admin.users.show', $w['id']) }}" class="lb-row">
                @if($w['image'])
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-[10.5px] font-bold text-slate-400">{{ $w['hits'] }} visits before leaving</p>
                </div>
...
{{-- ══════════════════════════════════════════════════════════════════
     7 · APPROVAL QUEUE + RENEWAL RADAR
════════════════════════════════════════════════════════════════════ --}}
<div class="grid grid-cols-1 xl:grid-cols-3 gap-4 md:gap-6 mb-5 md:mb-7">
    ...
    <div id="sec-queue" class="xl:col-span-2 card overflow-hidden scroll-mt-24">
        ...
        <div class="queue-item">
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center flex-wrap gap-1.5 mt-2.5">
                    ...
                    <form method="POST" action="{{ $approveUrl }}" class="inline">@csrf
                        <button type="submit" class="text-[11px] font-black px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-700 min-h-[32px] border border-emerald-100">
                            <i class="fas fa-check mr-1"></i>Approve
                        </button>
                    </form>
                    <form method="POST" action="{{ $rejectUrl }}" class="inline" onsubmit="return confirm('Reject this listing?')">@csrf
                        <button type="submit" class="text-[11px] font-black px-3 py-1.5 rounded-lg bg-rose-50 text-rose-700 min-h-[32px] border border-rose-100">
        ...
        @empty
            <div class="empty"><i class="fas fa-mug-hot"></i>Nothing is waiting for approval.</div>
        @endforelse
    </div>
    ...
    <div id="sec-renewals" class="card overflow-hidden scroll-mt-24">
        <div class="card-hd">
            <div><p class="eyebrow mb-1">Retention</p><h3 class="card-ttl">Renewal radar</h3></div>
        ...
        <div class="px-4 py-2.5 bg-amber-50 border-b border-amber-100 flex items-center gap-2.5">
            ...
            <p class="text-[11.5px] font-bold text-amber-800"><span class="num">{{ number_format($riskAmount) }} IQD</span> renews within 7 days</p>
        </div>
        @endif

        <div id="radar-expiring" class="max-h-[420px] overflow-y-auto custom-scrollbar">
            @forelse($expiringNow as $r)
            <div class="radar-item">
                <span class="dot st-{{ $r['state'] }}"></span>
                @if($r['image'])<img src="{{ $r['image'] }}" class="w-9 h-9 rounded-xl object-cover bg-slate-100" alt="">
                @else<div class="w-9 h-9 rounded-xl grid place-items-center text-white text-[11px] font-black" style="background:#303b97">{{ strtoupper(substr($r['name'],0,1)) }}</div>
                @endif
                <div class="flex-1 min-w-0">
                ...
                <div class="text-right shrink-0">
                    ...
                    <a href="{{ $link($r['type'] === 'agent' ? 'admin.agents.edit' : 'admin.offices.edit', $r['id']) }}"
                       class="block text-[10px] font-black mt-1" style="color:#303b97">Renew →</a>
                </div>
            </div>
            @empty
        ...
        <div id="radar-expired" class="hidden max-h-[420px] overflow-y-auto custom-scrollbar">
            @forelse($expiredNow as $r)
            <div class="radar-item">
                <span class="dot st-expired"></span>
                @if($r['image'])<img src="{{ $r['image'] }}" class="w-9 h-9 rounded-xl object-cover" alt="">
                @else<div class="w-9 h-9 rounded-xl grid place-items-center text-white text-[11px] font-black bg-red-500">{{ strtoupper(substr($r['name'],0,1)) }}</div>
                @endif
                <div class="flex-1 min-w-0">
                    <p class="text-[12.5px] font-bold text-slate-900 truncate">{{ $r['name'] }}</p>
                ...
                <div class="text-right shrink-0">
                    ...
                    <a href="{{ $link($r['type'] === 'agent' ? 'admin.agents.edit' : 'admin.offices.edit', $r['id']) }}"
                       class="block text-[10px] font-black mt-1" style="color:#303b97">Reactivate →</a>
                </div>
            </div>
            @empty
...
{{-- ══════════════════════════════════════════════════════════════════
     8 · INVENTORY — status donut, cities bar, most viewed
════════════════════════════════════════════════════════════════════ --}}
<div id="sec-inventory" class="grid grid-cols-1 xl:grid-cols-3 gap-4 md:gap-6 mb-5 md:mb-7 scroll-mt-24">
    <div class="card overflow-hidden">
    ...
    <div class="card overflow-hidden">
        ...
        @else
            <div class="empty" style="padding-top:60px"><i class="fas fa-map-location-dot"></i>No city data yet.</div>
        @endif
    </div>
    <div class="card overflow-hidden">
        <div class="card-hd"><div><p class="eyebrow mb-1">Last 7 days</p><h3 class="card-ttl">Most viewed listings</h3></div></div>
        <div class="p-2 max-h-[310px] overflow-y-auto custom-scrollbar">
            @forelse($topViewed as $i => $v)
            <a href="{{ $link('admin.properties.show', $v['id']) }}" class="lb-row">
                <span class="rank {{ $i===0 ? 'rank-1' : '' }}">{{ $i+1 }}</span>
                @if($v['image'])<img src="{{ asset($v['image']) }}" class="w-10 h-10 rounded-xl object-cover bg-slate-100" alt="">
                @else<div class="w-10 h-10 rounded-xl grid place-items-center bg-slate-100 text-slate-300"><i class="fas fa-image text-[11px]"></i></div>
                @endif
                <div class="flex-1 min-w-0">
                    <p class="text-[12.5px] font-bold text-slate-900 truncate">{{ $v['name'] }}</p>
                    <p class="text-[10.5px] font-bold text-slate-400">${{ number_format($v['price']) }}</p>
                </div>
                <div class="text-right shrink-0">
            ...
            @empty
                <div class="empty"><i class="fas fa-eye"></i>No view data yet.</div>
            @endforelse
        </div>
    </div>
</div>
...
@php
    ...
    $revItems  = [
        ...
        ['New this month',  $stats['this_month_revenue'],          $stats['new_subscriptions_this_month'] . ' subscriptions started', '#10b981',
            ($deltas['revenue'] >= 0 ? '+' : '') . $deltas['revenue'] . '% vs last month'],
    ];
@endphp
<div id="sec-revenue" class="card mb-5 md:mb-7 scroll-mt-24">
    <div class="card-hd">
    ...
    <div class="p-4 md:p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            @foreach($revItems as [$label, $amount, $sub, $color, $note])
            <div class="border border-slate-100 rounded-2xl p-4 md:p-5">
                <div class="flex items-center gap-2 mb-2.5">
                    <span class="w-2.5 h-2.5 rounded-full" style="background:{{ $color }}"></span>
                    <span class="eyebrow !text-slate-500">{{ $label }}</span>
                </div>
                <p class="text-[24px] md:text-[28px] font-black text-slate-900 num leading-none mb-1">
                    {{ number_format($amount) }} <span class="text-xs font-bold text-slate-400">IQD</span>
                </p>
...
{{-- ══════════════════════════════════════════════════════════════════
     10 · PEOPLE — leaderboard, viewings today, activity feed, new users
════════════════════════════════════════════════════════════════════ --}}
<div id="sec-people" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 md:gap-6 scroll-mt-24">
    ...
    <div class="card overflow-hidden">
        ...
        <div class="p-2 max-h-[360px] overflow-y-auto custom-scrollbar" id="lb-agents">
            ...
            <a href="{{ $link('admin.agents.show', $a['id']) }}" class="lb-row">
                <span class="rank {{ $i===0 ? 'rank-1' : '' }}">{{ $i+1 }}</span>
                @if($a['image'])<img src="{{ $a['image'] }}" class="w-9 h-9 rounded-full object-cover bg-slate-100" alt="">
                @else<div class="w-9 h-9 rounded-full grid place-items-center text-white text-[11px] font-black" style="background:#303b97">{{ strtoupper(substr($a['name'],0,1)) }}</div>
                @endif
                <div class="flex-1 min-w-0">
                    <p class="text-[12.5px] font-bold text-slate-900 truncate">{{ $a['name'] }}</p>
                    <p class="text-[10.5px] font-bold text-slate-400 truncate">{{ $a['sub'] }}</p>
                </div>
                <div class="text-right shrink-0">
                    <span class="block text-[13.5px] font-black text-slate-900 num">{{ $a['total'] }}</span>
                    @if($a['weekly'] > 0)<span class="text-[9.5px] font-black text-emerald-600">+{{ $a['weekly'] }} wk</span>
                    @else<span class="text-[9.5px] font-bold text-slate-300 uppercase">listings</span>@endif
                </div>
            </a>
            @empty<div class="empty"><i class="fas fa-user-tie"></i>No agents yet.</div>
            @endforelse
        </div>
        <div class="p-2 hidden max-h-[360px] overflow-y-auto custom-scrollbar" id="lb-offices">
            @forelse($topOffices as $i => $o)
            <a href="{{ $link('admin.offices.show', $o['id']) }}" class="lb-row">
                <span class="rank {{ $i===0 ? 'rank-1' : '' }}">{{ $i+1 }}</span>
                @if($o['image'])<img src="{{ $o['image'] }}" class="w-9 h-9 rounded-xl object-cover bg-slate-100" alt="">
                @else<div class="w-9 h-9 rounded-xl grid place-items-center text-white text-[11px] font-black" style="background:#6C3FC5">{{ strtoupper(substr($o['name'],0,1)) }}</div>
                @endif
                <div class="flex-1 min-w-0">
                    <p class="text-[12.5px] font-bold text-slate-900 truncate">{{ $o['name'] }}</p>
                    <p class="text-[10.5px] font-bold text-slate-400 truncate">{{ $o['sub'] }}</p>
                </div>
                <div class="text-right shrink-0">
                    <span class="block text-[13.5px] font-black text-slate-900 num">{{ $o['total'] }}</span>
                    @if($o['weekly'] > 0)<span class="text-[9.5px] font-black text-emerald-600">+{{ $o['weekly'] }} wk</span>
                    @else<span class="text-[9.5px] font-bold text-slate-300 uppercase">listings</span>@endif
                </div>
            </a>
            @empty<div class="empty"><i class="fas fa-building"></i>No offices yet.</div>
            @endforelse
        </div>
    </div>
    ...
    <div class="card overflow-hidden">
        ...
        <div class="p-2 max-h-[360px] overflow-y-auto custom-scrollbar">
            ...
            <a href="{{ $link('admin.appointments.show', $a['id']) }}" class="lb-row">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-[10.5px] font-bold text-slate-400 truncate">with {{ $a['user'] }}</p>
                </div>
            ...
            @empty<div class="empty"><i class="far fa-calendar"></i>No viewings booked today.</div>
            @endforelse
        </div>
    </div>
    ...
    <div class="card overflow-hidden">
        ...
        @php $tones = ['blue'=>['#eff6ff','#2563eb'],'indigo'=>['#f2f3fb','#303b97'],'amber'=>['#fffbeb','#d97706'],'violet'=>['#f5f3ff','#7c3aed']]; @endphp
        <div class="p-4 md:p-5 max-h-[360px] overflow-y-auto custom-scrollbar">
            ...
            @empty<div class="empty"><i class="fas fa-wave-square"></i>No activity recorded yet.</div>
            @endforelse
        </div>
    </div>
    ...
    <div class="card overflow-hidden">
        ...
        <div class="p-2 max-h-[360px] overflow-y-auto custom-scrollbar">
            ...
            <a href="{{ $link('admin.users.show', $user->id) }}" class="lb-row">
                @if($user->photo_image)<img src="{{ $user->photo_image }}" class="w-9 h-9 rounded-full object-cover bg-slate-100" alt="">
                @else<div class="w-9 h-9 rounded-full grid place-items-center bg-slate-100 text-slate-500 text-[11px] font-black">{{ strtoupper(substr($user->username ?? 'U', 0, 1)) }}</div>
                @endif
                <div class="flex-1 min-w-0">
            ...
            @empty<div class="empty"><i class="fas fa-user-plus"></i>No users yet.</div>
            @endforelse
        </div>
    </div>
</div>
...
(function () {
    ...
    /* ── Sparklines ─────────────────────────────────────────────── */
    const sparkDefs = [
        ['#spark-revenue',    series.revenue,    '#10b981'],
        ['#spark-users',      series.users,      BRAND],
        ['#spark-properties', series.properties, '#8b5cf6'],
        ['#spark-network',    series.properties.map((v, i) => v + series.users[i]), '#f59e0b'],
    ];
    sparkDefs.forEach(([sel, data, color]) => {
    ...
    /* ── Growth chart ───────────────────────────────────────────── */
    let metric = 'users', range = mobile ? 6 : 12;
    const META = {
        users:      { name: 'New users',    color: BRAND,     fmt: v => fmt(v) },
        properties: { name: 'New listings', color: '#8b5cf6', fmt: v => fmt(v) },
        revenue:    { name: 'Revenue',      color: '#10b981', fmt: v => fmt(v) + ' IQD' },
    };
    const growthOpts = () => ({
        series: [{ name: META[metric].name, data: series[metric].slice(-range) }],
        chart: { type: 'area', height: mobile ? 240 : 325, fontFamily: 'inherit', toolbar: { show: false }, zoom: { enabled: false } },
        colors: [META[metric].color],
        dataLabels: { enabled: false },
        stroke: { curve: 'smooth', width: 3 },
        fill: { type: 'gradient', gradient: { opacityFrom: .28, opacityTo: 0, stops: [0, 100] } },
        ...
        yaxis: { labels: { style: { colors: '#94a3b8', fontSize: '10.5px', fontWeight: 700 }, formatter: v => fmt(v) } },
        grid: { borderColor: '#f1f2f7', strokeDashArray: 5, padding: { left: 6, right: 8 } },
        tooltip: { y: { formatter: META[metric].fmt } },
    });

    let growth = null;
    const growthEl = document.querySelector('#growthChart');
    if (growthEl) { growth = new ApexCharts(growthEl, growthOpts()); growth.render(); }
    ...
    const seg = (sel, apply) => {
        document.querySelectorAll(sel + ' button').forEach(b => {
            b.addEventListener('click', () => {
                ...
                b.classList.add('on');
                apply(b);
                if (growth) growth.updateOptions(growthOpts(), true, true);
            });
        });
    };
    seg('#metricSeg', b => { metric = b.dataset.metric; });
    seg('#rangeSeg',  b => { range  = parseInt(b.dataset.range, 10); });
    ...
    if (visitorEl && visitors.available) {
        ...
        new ApexCharts(visitorEl, {
            ...
            chart: { type: 'bar', stacked: true, height: mobile ? 215 : 265, fontFamily: 'inherit', toolbar: { show: false } },
            colors: [BRAND, '#c7cbe8'],
    ...
    if (returnEl && retention.available) {
        ...
        new ApexCharts(returnEl, {
            ...
            chart: { type: 'bar', stacked: true, height: mobile ? 200 : 250, fontFamily: 'inherit', toolbar: { show: false } },
            colors: [BRAND, '#c7cbe8'],
            plotOptions: { bar: { borderRadius: 5, columnWidth: '58%' } },
            dataLabels: { enabled: false },
            legend: { show: false },
    ...
    const showGroup = name => {
        ...
        set('retRate', g.rate + '%');
        set('retReturning', new Intl.NumberFormat().format(g.returning));
        set('retMonth', new Intl.NumberFormat().format(g.month));
        set('retScopeWord', WORD[name] || 'people');
        set('retToday', new Intl.NumberFormat().format(g.today));
        set('retWeek', new Intl.NumberFormat().format(g.week));
        set('retMonthTile', new Intl.NumberFormat().format(g.month));
        set('retBack', new Intl.NumberFormat().format(g.returning));
        set('retLoyal', new Intl.NumberFormat().format(g.loyal));
    ...
    document.querySelectorAll('#retSeg button').forEach(b => {
        b.addEventListener('click', () => {
    ...
    if (statusEl) {
        ...
        new ApexCharts(statusEl, {
            ...
            chart: { type: 'donut', height: mobile ? 255 : 290, fontFamily: 'inherit' },
            ...
            plotOptions: { pie: { donut: { size: '72%', labels: {
                ...
                value: { fontSize: '24px', fontWeight: 900, color: '#0f172a', formatter: v => fmt(v) },
    ...
    if (cityEl && cities.length) {
        new ApexCharts(cityEl, {
            ...
            chart: { type: 'bar', height: mobile ? 255 : 290, fontFamily: 'inherit', toolbar: { show: false } },
            colors: [BRAND],
    ...
    /* ── Tab switchers ──────────────────────────────────────────── */
    const wireTabs = (container, attr, prefix) => {
