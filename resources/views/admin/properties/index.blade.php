        ...
        <div class="flex space-x-3">
            <a href="{{ route('admin.properties.viewers', $property->id) }}"
               class="bg-indigo-500 text-white px-6 py-3 rounded-xl font-semibold hover:shadow-lg transition">
                <i class="fas fa-users mr-2"></i> Viewers ({{ $property->interactions->where('interaction_type', 'view')->count() }})
            </a>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm p-6 border border-gray-100 mb-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <p class="text-base font-semibold text-gray-800">{{ $property->type['category'] ?? 'N/A' }}</p>
                </div>
                <div>
                ...
                <div>
                    ...
                    <p class="text-base font-semibold text-gray-800">{{ $property->rooms['bedroom'] ?? 0 }} Bed, {{ $property->rooms['bathroom'] ?? 0 }} Bath</p>
                </div>
                <div>
    ...
    <div class="space-y-6">
                    ...
                    <p class="text-base font-semibold text-gray-800">
                        @php
                            ...
                            if ($property->owner_type === 'App\Models\Agent') {
                                $ownerName = $property->owner->agent_name ?? 'Unknown Agent';
                            } elseif ($property->owner_type === 'App\Models\RealEstateOffice') {
                                $ownerName = $property->owner->company_name ?? 'Unknown Office';
                            } elseif ($property->owner_type === 'App\Models\User') {
                                $ownerName = $property->owner->username ?? 'Unknown User';
                            }
                        @endphp
                        {{ $ownerName }}
                    </p>
        ...
        <!-- Property Viewers -->
        @if($property->interactions->where('interaction_type', 'view')->count() > 0)
        <div class="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
            <h3 class="text-lg font-bold text-gray-800 mb-4 flex items-center justify-between">
                ...
                <span class="text-sm font-normal text-gray-500">({{ $property->interactions->where('interaction_type', 'view')->count() }} total)</span>
            </h3>

            <div class="space-y-3 max-h-96 overflow-y-auto">
                @foreach($property->interactions->where('interaction_type', 'view')->take(20) as $interaction)
                    @if($interaction->user)
            ...
            @if($property->interactions->where('interaction_type', 'view')->count() > 20)
            <div class="mt-3 text-center">
                <a href="{{ route('admin.properties.viewers', $property->id) }}"
                   class="text-sm text-indigo-600 hover:text-indigo-700 font-semibold">
                    View All {{ $property->interactions->where('interaction_type', 'view')->count() }} Viewers
                </a>
            </div>
            @endif
        </div>
